#!/usr/bin/env python3
"""exp-004 local verdict analysis (CPU).

Inputs: out/per_era_numerai_corr.csv, out/per_era_spearman.csv,
out/diagnostics.json (pulled back from the cluster job), plus
<run-dir>/data/shard_verdict.parquet (for per-era target dispersion, F8).

Frozen machinery (binding):
...
F8: tail-era breakdown by baseline per-era corr quartiles and by per-era
    target-dispersion quartiles (descriptive; within-bucket eras are
    non-contiguous so bucket CIs use iid bootstrap, labeled as such).
F10: corr-Sharpe (mean/std of per-era corr) diff via the SAME paired
    block-bootstrap machinery (joint era-block resampling of both arms).
C4/GAF: same paired machinery vs filter_off, and filter_on vs each control.
"""
import json
...
ARMS = ["filter_off", "filter_on", "c4_random", "gaf"]
SEEDS = [0, 1, 2]
...
def main():
    ...
    # F9 sanity band on realized baseline
    ...
    print(f"\nF9: baseline nc {base:+.5f} vs band [{band[0]:.4f}, "
          f"{band[1]:.4f}] -> "
    ...
    pairs = [("filter_on", "filter_off"), ("c4_random", "filter_off"),
             ("gaf", "filter_off"), ("filter_on", "c4_random"),
             ("filter_on", "gaf")]
    ...
    report["verdict"] = {"category": verdict, "f12_downgrade_applied":
                         ...
                         "threshold_nc": THR_NC}
    print(f"\n=== VERDICT (filter_on vs filter_off, F3={THR_NC}): "
    ...
    # --- diagnostics engagement check (during verdict runs) ---
    ...
    for tag, d in diag["runs"].items():
        ...
        if "kept_energy_frac" in log[0]:
            ...
            entry["kept_energy_frac_range"] = [float(kef.min()),
                                               float(kef.max())]
        if "frac_coords_kept" in log[0]:
            fck = np.array([e["frac_coords_kept"] for e in log])
            entry["frac_coords_kept_median"] = float(np.median(fck))
    ...
    for t, v in sorted(eng.items()):
        print(f"  {t}: k_med {v['k_median']:.0f}, ratio_med "
              ...
              + (f", frac_coords_med {v['frac_coords_kept_median']:.5f}"
                 if "frac_coords_kept_median" in v else ""))
    print(f"engagement_ok (filter engaged, selective, non-degenerate "
          f"throughout): {engaged}")

    report["timings_s"] = diag["timings_s"]
    with open(OUT / "verdict_analysis.json", "w") as f:
        json.dump(report, f, indent=2, default=float)
    print("\nwrote out/verdict_analysis.json")
