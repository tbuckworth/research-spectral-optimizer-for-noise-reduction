...
for OUTER_NUMBER in 1 2 3; do
  OUTER_SELECTIONS+=("$PROJECT/results/selection-outer_${OUTER_NUMBER}-f2-top1.json")
  OUTER_AUDITS+=("$PROJECT/results/audit-outer_${OUTER_NUMBER}-u100000/outer-audit.json")
done
...
if [[ -e "$SELECTION" || -e "$MANIFEST" || -e "${MANIFEST}.tmp" ]] \
    || tmux has-session -t "$SUPERVISOR_SESSION" 2>/dev/null; then
...
for SEED in 0 1 2; do
  NUMERAI_REUSE_COMPLETE=1 bash "$PROJECT/slurm/submit-selected.sh" \
    "$SELECTION" 100000 "$SEED" "$DEPENDENCY_JOB" \
