...
if [[ $# -lt 6 ]]; then
  echo "usage: $0 PLAN_JSON confirmation|long-scout UPDATES SEED DEPENDENCY_JOB_ID SPLIT..." >&2
  exit 2
fi
PLAN=$1
MODE=$2
...
if [[ ! -f $PLAN || ! $UPDATES =~ ^(5000|20000|100000)$ \
      || ! $SEED =~ ^[0-9]+$ || ! $DEPENDENCY =~ ^[0-9]+$ \
      || ( $MODE != confirmation && $MODE != long-scout ) \
      || ( $MODE == confirmation && $UPDATES == 100000 ) \
      || ( $MODE == long-scout && $UPDATES != 100000 ) \
      || ( $REUSE_COMPLETE != 0 && $REUSE_COMPLETE != 1 ) ]]; then
  echo "invalid successive-plan submission arguments" >&2
  exit 1
fi
...
print(*value["confirmation_selections"][sys.argv[2]]["paired_union"], sep="\n")
' "$PLAN" "$UPDATES")
else
  mapfile -t IDS < <(python3 -c '
import json,sys
print(*json.load(open(sys.argv[1]))["long_scout_paired_union"], sep="\n")
' "$PLAN")
fi
[[ ${#IDS[@]} -gt 0 ]] || { echo "successive-plan selection is empty" >&2; exit 1; }
