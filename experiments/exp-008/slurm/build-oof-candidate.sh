...
for OUTER_NUMBER in 1 2 3; do
  SELECTION="$PROJECT/results/selection-outer_${OUTER_NUMBER}-f2-top1.json"
  AUDIT="$PROJECT/results/audit-outer_${OUTER_NUMBER}-u100000/outer-audit.json"
...
if (audit.get("status") != "audit_complete"
        ...
        or len(ids) != 1 or audit.get("selected", {}).get(arm) != ids[0]):
    raise SystemExit("selection and completed audit do not agree")
print(ids[0])
' "$SELECTION" "$AUDIT" "$ARM" "outer_${OUTER_NUMBER}")
    for SEED in 0 1 2; do
      RESULT="$PROJECT/results/stage-outer_${OUTER_NUMBER}-u100000-s${SEED}-${ARM}-c${CONFIG_ID}/result.json"
      if [[ ! -f $RESULT ]]; then
