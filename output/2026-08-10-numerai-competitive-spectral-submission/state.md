...
The search plan was generated and approved autonomously under the user's instruction to proceed. Three disjoint searches are running: official Numerai sources, methodological/academic sources, and high-quality community evidence. Preliminary official-source findings fix the current dataset at v5.3, the payout target at `target_cyrus_20`, the primary offline score at exact CORR20v2, and the official benchmark walk-forward geometry at 156-era validation blocks with an 8-era purge for 20D targets. The existing local NumPy shard has no provenance metadata and is therefore excluded from final comparable claims unless independently reconstructed.
