...
WD = 1e-3          # t07
DROPOUT = 0.2      # t07
SEEDS = [10, 11, 12]  # AUDIT: fresh seeds
MP_FACTOR = 2.0    # F2
LOG_EVERY = 50
EVAL_CHUNK = 65536
MIN_ERA_ROWS = 100
...
def kept_energy_fraction(G, mp_factor):
    ...
    except Exception:  # AUDIT-PATCH: same cusolver non-convergence, CPU fp64
        print("AUDIT-PATCH: diagnostic eigvalsh fell back to CPU fp64",
              flush=True)
        ev = torch.linalg.eigvalsh(S.detach().double().cpu()).flip(0)\
            .clamp(min=0).to(S.dtype).to(S.device)
    tot = ev.sum()
    if tot < 1e-12:
        return 0.0
...
def main():
    ...
    ap.add_argument("--verdict-shard2", required=True,
                    help="audit-built independent row subsample")
    ...
    Xev2, yev2, era_ev2, eras_ev2 = load_shard(args.verdict_shard2,
                                               verdict_allowed,
                                               "AUDIT shard (own subsample)")
    yev2_np = yev2.cpu().numpy()
    ...
    per_era_nc2 = pd.DataFrame({"era": pe_zero["era"]})
    ...
    for seed in seeds:
        ...
        for arm in ARM_ORDER:
            ...
            pe2 = per_era_metrics(predict(model, Xev2), yev2_np,
                                  era_ev2, eras_ev2)
            per_era_nc2[tag] = pe2["numerai_corr"].to_numpy()
    ...
    per_era_nc2.to_csv(outdir / "per_era_numerai_corr_AUDITSHARD.csv",
                       index=False)
