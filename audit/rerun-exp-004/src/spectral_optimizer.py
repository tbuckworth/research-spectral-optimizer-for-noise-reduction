...
class SpectralConsensusFilter:
    ...
    def _spectral_filter(self, G, batch_size):
        ...
        except Exception:  # AUDIT-PATCH: cusolver eigh non-convergence on
            # ill-conditioned S (hit at fresh seed 10). Same math, CPU fp64.
            print("AUDIT-PATCH: eigh fell back to CPU fp64", flush=True)
            ev64, U64 = torch.linalg.eigh(S.detach().double().cpu())
            eigenvalues, U = ev64.to(S.dtype).to(S.device), \
                U64.to(S.dtype).to(S.device)
